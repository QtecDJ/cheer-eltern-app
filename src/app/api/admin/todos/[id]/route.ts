...
export async function PATCH(req: Request, context: any) {
  ...
  try {
    ...
    const updated = await updateTodo(id, { title: body.title, description: body.description ?? null, priority: body.priority, dueDate: body.dueDate ? new Date(body.dueDate) : null });
    return NextResponse.json({ success: true, todo: updated });
  } catch (e) {
    console.error(e);
    return NextResponse.json({ error: "server_error" }, { status: 500 });
